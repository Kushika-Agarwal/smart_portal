...
function ApplicationForm() {
  ...
  const validate = () => {
    ...
    if (!formData.urcCode.trim())
      newErrors.urcCode = "Enter the URC code";

    if (!formData.query.trim())
      newErrors.query = "Write your query here";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(
        // "http://localhost:5000/api/application/submit",
      ...
      <div className="w-full flex justify-center items-center py-12 px-4">
        ...
        >
          ...
          <label className="flex items-center font-medium">
            Application ID
            <InfoButton text="Application ID information" />
          </label>
          ...
          <label className="flex items-center font-medium">
            Payment Transfer No.
            <InfoButton text="Payment transfer info" />
          </label>
          ...
          {errors.paymentTransferNo &&
            <p className="text-red-500 text-sm mb-3">{errors.paymentTransferNo}</p>
          }

          {/* URC Code */}
          <label className="flex items-center font-medium">
            URC Code
            <InfoButton text="URC code info" />
          </label>
          ...
          <div className="flex justify-between text-sm text-gray-500 mb-1">
            ...
            <span>{formData.query.length}/{maxQueryLength}</span>
          </div>
